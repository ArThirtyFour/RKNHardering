...
|-----------|---------------|--------------|
| `127.x.x.x` / `::1` | Localhost — локальный DNS-резолвер VPN | да |
| `10.x.x.x` / `172.16–31.x.x` / `192.168.x.x` / `100.64–127.x.x` / `fc00::/7` | Частный DNS: подозрителен только если не совпадает с реальной private/ULA-подсетью основного интерфейса | зависит |
| `169.254.x.x` / `fe80::/10` | Link-local | нет |
